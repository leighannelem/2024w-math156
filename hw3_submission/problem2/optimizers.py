...
class RMSprop:
    ...
    def update(self, grad_W, grad_b):
        ...
        ### use the right update rules (the structure should be similar to Adam/SGDmomentum)
        ...
        self.param_W = grad_W /(np.sqrt(self.cache_W) + self.eps)
        self.param_b = grad_b /(np.sqrt(self.cache_b) + self.eps)
